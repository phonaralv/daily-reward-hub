...
## 3단계 — 저사양 모바일 기본 대응 (감지까지만)

이번 PR에서는 **감지 모듈 신설까지만** 진행하고, 실제 엔진 연결은 분리.
...
- `src/shared/lib/perf/deviceTier.ts` 신설
  ...
  - `useDeviceTier()` 훅 + `getDeviceTier()` 순수 함수 export.
- 사용처 연결 없음. 기존 `liveEngine.ts`의 `isLowEnd()`는 그대로 둠.
- 단위 동작 확인: 콘솔에서 `getDeviceTier()` 결과가 SSR/CSR 모두에서 throw 없이 반환되는지만 확인.

후속(별도 작업)으로 미룸:
- `liveEngine.ts` `isLowEnd()` → `deviceTier` 교체
- `useGlobalPulse`의 intensity 강등 연결
- seed-deterministic 초기화
